...
class TiebaSPider:
    ...
    def get_content_list(self, html):  #获取列表页面数据
    ...
    def get_img_list(self, html): #获取图片
        ...
        next_url = html.xpath("//a[text()='下一页']/@href") #type list
        img_list += self.get_img_next(next_url)
        return img_list

    def get_img_next(self, next_url): #获取下一页的图片
        total_imgs = []
        while len(next_url) > 0: #根据list长度,判断是否有下一页
            next_url = "http://tieba.baidu.com/mo/q----,sz@320_240-1-3---2/"+next_url[0]
